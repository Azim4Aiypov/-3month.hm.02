...
def main(page: ft.Page):
    ...
    def load_tasks():
        ...
        for task_id, task_text, completed, created_at, in_progress in main_db.get_tasks(filter_type):
            task_list.controls.append(create_task_row(task_id, task_text, created_at, completed, in_progress))
        page.update()

    def create_task_row(task_id, task_text, created_at, completed, in_progress):
    ...
    clear_button = ft.ElevatedButton("Очистить выполненные", on_click=clear_done, icon=ft.icons.DELETE)
    ...
    sort_by_status = ft.ElevatedButton("Сортировать по статусу", on_click=sort_status, icon=ft.icons.CHECK_CIRCLE)

    filter_button = ft.Row([
        ft.ElevatedButton("Все задачи", on_click=lambda e: set_filter("all")),
        ft.ElevatedButton("Выполненные задачи", on_click=lambda e: set_filter("completed")),
        ft.ElevatedButton("Невыполненные задачи", on_click=lambda e: set_filter("incomplete")),
        ft.ElevatedButton("Задачи в работе", on_click=lambda e: set_filter("in_progress"))
    ...
    content = ft.Container(
        content=ft.Column([
            ...
            ft.Row([sort_by_date, sort_by_status], alignment=ft.MainAxisAlignment.CENTER),
            filter_button,
            task_list
        ], alignment=ft.MainAxisAlignment.CENTER),
        padding=20,
